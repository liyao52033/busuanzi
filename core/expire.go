...
// setExpireWithDuration
// @description set the expiration time of the key with custom duration
func setExpireWithDuration(duration time.Duration, key ...string) {
	if duration == 0 {
		return
	}
	...
	for _, k := range key {
		if config.DEBUG {
			log.Printf("[expire] Set %s expired with %d s \n", k, duration/time.Second)
		}
		_redis.Expire(ctx, k, duration)
	}
}
